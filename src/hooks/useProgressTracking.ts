...
export function useProgressTracking(studentId: string) {
  ...
  const calculateMilestones = (history: HistoryEntry[]): ProgressMilestone[] => {
  ...
  const analyzeLearningPath = (history: HistoryEntry[]): LearningPath => {
    ...
    history.forEach(attempt => {
      const key = `${attempt.levelName}-${attempt.weekNo}`;
      if (!topicMap.has(key)) {
    ...
    const currentLevel = mostRecent.levelName;
  ...
  const analyzeProgressSessions = (history: HistoryEntry[]): ProgressSession[] => {
    if (!history || history.length === 0) return [];
  ...
  const analyzeSkillProgression = (history: HistoryEntry[]): SkillProgression[] => {
    if (!history || history.length === 0) return [];
    ...
    history.forEach(attempt => {
      if (!skillMap.has(attempt.levelName)) {
        skillMap.set(attempt.levelName, []);
      }
      skillMap.get(attempt.levelName)!.push(attempt);
    });
  ...
  const generateWeeklyGoals = (history: HistoryEntry[]): WeeklyGoals => {
