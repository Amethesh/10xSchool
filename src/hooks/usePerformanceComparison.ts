...
export function usePerformanceComparison(
  ...
  const leaderboardQuery = useQuery({
    ...
    queryFn: () => level && weekNo && difficulty 
  ...
  const compareAttempts = (history: any[], currentId?: string): AttemptComparison | null => {
    ...
    if (currentId) {
      ...
      previousAttempt = currentIndex < history.length - 1 ? history[currentIndex + 1] : undefined;
    } else {
    ...
    const currentMetrics = {
      ...
      accuracy: (currentAttempt.correctAnswers / currentAttempt.totalQuestions) * 100,
      speed: currentAttempt.totalQuestions / (currentAttempt.timeSpent / 60),
    ...
    const previousMetrics = previousAttempt ? {
      ...
      accuracy: (previousAttempt.correctAnswers / previousAttempt.totalQuestions) * 100,
      speed: previousAttempt.totalQuestions / (previousAttempt.timeSpent / 60),
    ...
    if (previousMetrics) {
      ...
      if (Math.abs(improvements.scoreChange) >= 10) {
        insights.push({
          type: improvements.scoreChange > 0 ? 'improvement' as const : 'regression' as const,
          category: 'score' as const,
          message: `Score ${improvements.scoreChange > 0 ? 'improved' : 'decreased'} by ${Math.abs(improvements.scoreChange).toFixed(1)}%`,
          significance: Math.abs(improvements.scoreChange) >= 20 ? 'high' as const : 'medium' as const,
        });
      }

      // Speed insights
      if (Math.abs(improvements.speedChange) >= 0.5) {
        insights.push({
          type: improvements.speedChange > 0 ? 'improvement' as const : 'regression' as const,
          category: 'speed' as const,
          message: `Speed ${improvements.speedChange > 0 ? 'increased' : 'decreased'} by ${Math.abs(improvements.speedChange).toFixed(1)} questions/min`,
          significance: Math.abs(improvements.speedChange) >= 1 ? 'high' as const : 'medium' as const,
        });
      }
      ...
      if (Math.abs(improvements.accuracyChange) >= 5) {
        insights.push({
          type: improvements.accuracyChange > 0 ? 'improvement' as const : 'regression' as const,
          category: 'accuracy' as const,
          message: `Accuracy ${improvements.accuracyChange > 0 ? 'improved' : 'decreased'} by ${Math.abs(improvements.accuracyChange).toFixed(1)}%`,
          significance: Math.abs(improvements.accuracyChange) >= 10 ? 'high' as const : 'medium' as const,
        });
      }
    }
  ...
  const comparePeers = (
    ...
    const leaderboardAverage = leaderboard.length > 0
    ...
    const similarPerformers = leaderboard
      .filter(entry => Math.abs(entry.score - studentScore) <= 10 && entry.studentId !== studentId)
      .slice(0, 5)
      .map(entry => ({
        rank: entry.rank,
        score: entry.score,
  ...
  const compareHistoricalPeriods = (history: any[], timeframe: 'week' | 'month' | 'all_time'): HistoricalComparison => {
    ...
    const currentPeriodAttempts = history.filter(h => 
      new Date(h.completedAt) >= periodStart
    );

    const previousPeriodAttempts = timeframe !== 'all_time' ? history.filter(h => {
    ...
    const previousPeriod = previousPeriodAttempts.length > 0 
      ? calculatePeriodStats(previousPeriodAttempts) 
      : undefined;

    const trends = previousPeriod ? {
      scoreImprovement: currentPeriod.averageScore - previousPeriod.averageScore,
      activityChange: currentPeriod.quizzesCompleted - previousPeriod.quizzesCompleted,
      efficiencyChange: (currentPeriod.averageScore / (currentPeriod.totalTimeSpent || 1)) - 
                       (previousPeriod.averageScore / (previousPeriod.totalTimeSpent || 1)),
      consistencyChange: previousPeriod.consistency - currentPeriod.consistency, // Lower is better
    } : {
      scoreImprovement: 0,
      activityChange: 0,
      efficiencyChange: 0,
    ...
    scoreThresholds.forEach(threshold => {
      ...
      if (firstAchievement) {
        milestones.push({
          ...
          significance: threshold >= 80 ? 'major' as const : 'minor' as const,
        });
      }
    });
  ...
  const compareDifficulties = (history: any[]): DifficultyComparison => {
    ...
    if (difficultyStats.easy.averageScore < 70 && difficultyStats.easy.attempts > 0) {
    ...
    if (difficultyStats.medium.averageScore >= 80 && difficultyStats.hard.attempts < 3) {
      recommendations.push({
  ...
  const compareLevels = (history: any[]): LevelComparison => {
    ...
    const sortedByScore = [...levels].sort((a, b) => b.averageScore - a.averageScore);
    const strongestLevel = sortedByScore[0]?.level || '';
    const weakestLevel = sortedByScore[sortedByScore.length - 1]?.level || '';

    // Calculate improvement (simplified)
    const mostImprovedLevel = levels.reduce((best, current) => {
      return current.averageScore > (best?.averageScore || 0) ? current : best;
    }, null as any)?.level || '';
    ...
    const recommendations = levels
      ...
      .map(l => ({
        ...
        reason: `Average score of ${l.averageScore.toFixed(1)}% needs improvement`,
      }));
  ...
  const attemptComparison = historyQuery.data ? compareAttempts(historyQuery.data, currentAttemptId) : null;
  
  const peerComparison = (leaderboardQuery.data && rankingQuery.data && historyQuery.data) 
    ? comparePeers(leaderboardQuery.data, rankingQuery.data, historyQuery.data[0]?.score || 0)
    : null;

  const weeklyComparison = historyQuery.data ? compareHistoricalPeriods(historyQuery.data, 'week') : null;
  const monthlyComparison = historyQuery.data ? compareHistoricalPeriods(historyQuery.data, 'month') : null;
  const allTimeComparison = historyQuery.data ? compareHistoricalPeriods(historyQuery.data, 'all_time') : null;

  const difficultyComparison = historyQuery.data ? compareDifficulties(historyQuery.data) : null;
  const levelComparison = historyQuery.data ? compareLevels(historyQuery.data) : null;
  ...
  return {
    ...
    // Loading states
    isLoading: historyQuery.isLoading || leaderboardQuery.isLoading || rankingQuery.isLoading,
