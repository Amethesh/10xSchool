...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl bg-white/90 backdrop-blur-md rounded-3xl shadow-sm border border-white/20 px-4 md:px-6 py-3 flex items-center justify-between z-50 transition-all duration-300">
      ...
      <ul className="hidden md:flex items-center space-x-8 lg:space-x-12">
        {NAV_LINKS.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className={`text-black text-sm lg:text-md font-semibold hover:text-[#0246A4] relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 after:bg-[#0246A4] after:transition-all after:duration-300 hover:after:w-full ${pathname === link.href ? "text-[#0246A4] after:w-full" : ""
                }`}
            >
              {link.label}
            </Link>
      ...
      <div
        className={`fixed top-full left-0 w-full mt-2 px-2 transition-all duration-300 ease-in-out transform origin-top md:hidden ${isMenuOpen
          ...
          <ul className="flex flex-col items-center gap-4 w-full">
            {NAV_LINKS.map((link) => (
              <li key={link.href} className="w-full text-center">
                <Link
                  href={link.href}
                  className={`block w-full py-2 text-lg font-semibold transition-colors ${pathname === link.href ? "text-[#0246A4]" : "text-gray-800 hover:text-[#0246A4]"
                    }`}
